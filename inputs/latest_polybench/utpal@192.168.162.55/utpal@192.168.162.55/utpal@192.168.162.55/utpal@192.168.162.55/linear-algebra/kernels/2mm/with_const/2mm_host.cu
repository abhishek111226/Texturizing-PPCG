...
#define cudaCheckKernel() \
  do { \
    ...
    cudaMallocArray(&cuArr_B, &channelDesc_B, 900, 1100);
    cudaMemcpyToArray(cuArr_B, 0, 0, B, (1100) * (900) * sizeof(float), cudaMemcpyHostToDevice);
    cudaBindTextureToArray(texRef_B, cuArr_B, channelDesc_B);
    
    cudaArray* cuArr_C;
    cudaChannelFormatDesc channelDesc_C= cudaCreateChannelDesc<float>();
    cudaMallocArray(&cuArr_C, &channelDesc_C, 1200, 900);
    cudaMemcpyToArray(cuArr_C, 0, 0, C, (900) * (1200) * sizeof(float), cudaMemcpyHostToDevice);
    cudaBindTextureToArray(texRef_C, cuArr_C, channelDesc_C);
