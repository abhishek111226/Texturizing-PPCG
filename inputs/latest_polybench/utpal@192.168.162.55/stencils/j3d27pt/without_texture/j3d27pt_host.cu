...
#define cudaCheckKernel() \
  do { \
    cudaCheckReturn(cudaGetLastError()); \
	   ...
	   float (*dev_A)[1000][1000];
	   float (*dev_B)[1000][1000];
	   
	   cudaCheckReturn(cudaMalloc((void **) &dev_A, (1000) * (1000) * (1000) * sizeof(float)));
	   cudaCheckReturn(cudaMalloc((void **) &dev_B, (999) * (1000) * (1000) * sizeof(float)));
	   
	   
	   cudaCheckReturn(cudaMemcpy(dev_A, A, (1000) * (1000) * (1000) * sizeof(float), cudaMemcpyHostToDevice));
