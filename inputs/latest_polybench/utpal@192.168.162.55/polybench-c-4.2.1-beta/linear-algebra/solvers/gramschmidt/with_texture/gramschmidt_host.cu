...
#define cudaCheckKernel() \
  do { \
    ...
    cudaArray* cuArr_Q;
    cudaChannelFormatDesc channelDesc_Q= cudaCreateChannelDesc<float>();
    cudaMallocArray(&cuArr_Q, &channelDesc_Q, 1200, 1000, cudaArraySurfaceLoadStore);
    cudaMemcpyToArray(cuArr_Q, 0, 0, Q, (1000) * (1200) * sizeof(float), cudaMemcpyHostToDevice);
    cudaBindSurfaceToArray(surfRef_Q, cuArr_Q, channelDesc_Q);
      ...
      if ((c0 - 3597) % 5995 == 0) {
        {
          dim3 k2_dimBlock(32);
          dim3 k2_dimGrid(32);
          kernel2 <<<k2_dimGrid, k2_dimBlock>>> (dev_A, dev_R, c0);
          cudaCheckKernel();
        }
      ...
      if (5995 * ((c0 + 1199) / 2398) + 3591005 >= 3 * c0 && 5995 * ((2 * c0 + 2398) / 3597) >= 3 * c0 + 5995 && c0 % 1199 == 0)
        {
          ...
          kernel4 <<<k4_dimGrid, k4_dimBlock>>> (dev_A, dev_R, c0);
          cudaCheckKernel();
        }
      ...
      if (5995 * (c0 / 2398) + 3593403 >= 3 * c0 && 5995 * ((2 * c0 + 1199) / 3597) >= 3 * c0 + 3597 && c0 % 1199 == 0)
        {
          dim3 k5_dimBlock(32);
          dim3 k5_dimGrid(75);
          kernel5 <<<k5_dimGrid, k5_dimBlock>>> (dev_A, dev_R, c0);
          cudaCheckKernel();
        }
    ...
    cudaMemcpyFromArray(Q, cuArr_Q, 0, 0, (1000) * (1200) * sizeof(float),  cudaMemcpyDeviceToHost);
    
    cudaCheckReturn(cudaMemcpy(R, dev_R, (1200) * (1200) * sizeof(float), cudaMemcpyDeviceToHost));
    
    
    cudaFreeArray(cuArr_Q);
