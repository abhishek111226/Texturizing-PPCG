...
#define cudaCheckKernel() \
  do { \
    ...
    double (*dev_grid)[262][1028][9];
    
    cudaCheckReturn(cudaMalloc((void **) &dev_grid, (2) * (262) * (1028) * (9) * sizeof(double)));
    
    
    cudaCheckReturn(cudaMemcpy(dev_grid, grid, (2) * (262) * (1028) * (9) * sizeof(double), cudaMemcpyHostToDevice));
    for (int c0 = 0; c0 < _nTimesteps; c0 += 1)
      {
        dim3 k0_dimBlock(16, 32);
        dim3 k0_dimGrid(ppcg_min(256, (_nX + 31) / 32), ppcg_min(256, (_nY + 31) / 32));
        kernel0 <<<k0_dimGrid, k0_dimBlock>>> (dev_grid, _nTimesteps, _nY, _nX, c0);
        cudaCheckKernel();
      }
      
    cudaCheckReturn(cudaMemcpy(grid, dev_grid, (2) * (262) * (1028) * (9) * sizeof(double), cudaMemcpyDeviceToHost));
