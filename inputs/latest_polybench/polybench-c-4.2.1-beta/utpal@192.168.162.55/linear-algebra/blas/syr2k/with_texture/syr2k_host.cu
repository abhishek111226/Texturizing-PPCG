...
#define cudaCheckKernel() \
  do { \
    ...
    cudaMallocArray(&cuArr_A, &channelDesc_A, 1000, 1200);
    cudaMemcpyToArray(cuArr_A, 0, 0, A, (1200) * (1000) * sizeof(float), cudaMemcpyHostToDevice);
    cudaBindTextureToArray(texRef_A, cuArr_A, channelDesc_A);
    
    cudaArray* cuArr_B;
    cudaChannelFormatDesc channelDesc_B= cudaCreateChannelDesc<float>();
    cudaMallocArray(&cuArr_B, &channelDesc_B, 1000, 1200);
    cudaMemcpyToArray(cuArr_B, 0, 0, B, (1200) * (1000) * sizeof(float), cudaMemcpyHostToDevice);
    ...
    cudaCheckReturn(cudaMemcpy(dev_C, C, (1200) * (1200) * sizeof(float), cudaMemcpyHostToDevice));
    {
      ...
      kernel0 <<<k0_dimGrid, k0_dimBlock>>> (dev_C, alpha, beta);
      cudaCheckKernel();
    }
