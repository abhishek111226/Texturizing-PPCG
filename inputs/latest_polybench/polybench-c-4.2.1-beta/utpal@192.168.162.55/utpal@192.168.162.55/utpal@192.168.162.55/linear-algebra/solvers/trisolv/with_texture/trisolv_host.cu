...
#define cudaCheckKernel() \
  do { \
    ...
    cudaArray* cuArr_L;
    cudaChannelFormatDesc channelDesc_L= cudaCreateChannelDesc<float>();
    cudaMallocArray(&cuArr_L, &channelDesc_L, 2000, 2000);
    cudaMemcpyToArray(cuArr_L, 0, 0, L, (2000) * (2000) * sizeof(float), cudaMemcpyHostToDevice);
    cudaBindTextureToArray(texRef_L, cuArr_L, channelDesc_L);
      ...
      if (c0 % 2 == 0)
        {
          dim3 k1_dimBlock;
          dim3 k1_dimGrid;
          kernel1 <<<k1_dimGrid, k1_dimBlock>>> (dev_x, c0);
          cudaCheckKernel();
        }
      ...
      if (c0 >= 1 && c0 <= 3997)
        {
          dim3 k2_dimBlock(32);
          dim3 k2_dimGrid(63);
          kernel2 <<<k2_dimGrid, k2_dimBlock>>> (dev_x, c0);
          cudaCheckKernel();
        }
    ...
    cudaUnbindTexture(texRef_L);
    
    cudaFreeArray(cuArr_L);
