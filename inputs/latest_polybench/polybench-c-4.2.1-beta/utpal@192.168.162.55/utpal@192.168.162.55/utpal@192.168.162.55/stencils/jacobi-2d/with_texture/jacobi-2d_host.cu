...
#define cudaCheckKernel() \
  do { \
    ...
    cudaMemcpyToArray(cuArr_A, 0, 0, A, (1300) * (1300) * sizeof(float), cudaMemcpyHostToDevice);
    ...
    for (int c0 = 0; c0 <= 499; c0 += 1) {
      ...
      
      {
        ...
        dim3 k1_dimGrid(41, 41);
        kernel1 <<<k1_dimGrid, k1_dimBlock>>> (c0);
        cudaCheckKernel();
      }
      
    }
    cudaMemcpyFromArray(A, cuArr_A, 0, 0, (1300) * (1300) * sizeof(float),  cudaMemcpyDeviceToHost);
    
    cudaMemcpyFromArray(B, cuArr_B, 0, 0, (1300) * (1300) * sizeof(float),  cudaMemcpyDeviceToHost);
