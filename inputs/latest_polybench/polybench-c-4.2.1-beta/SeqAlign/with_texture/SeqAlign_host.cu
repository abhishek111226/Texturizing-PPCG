...
#define cudaCheckKernel() \
  ...
  cudaCheckReturn(cudaMalloc((void **) &dev_M, (6000) * (4000) * sizeof(float)));
  
  
  cudaArray* cuArr_MatchQ;
  cudaChannelFormatDesc channelDesc_MatchQ= cudaCreateChannelDesc<float>();
  cudaMallocArray(&cuArr_MatchQ, &channelDesc_MatchQ, 4000, 6000);
  cudaMemcpyToArray(cuArr_MatchQ, 0, 0, MatchQ, (6000) * (4000) * sizeof(float), cudaMemcpyHostToDevice);
  cudaBindTextureToArray(texRef_MatchQ, cuArr_MatchQ, channelDesc_MatchQ);
  
  {
    dim3 k0_dimBlock(16, 32);
    dim3 k0_dimGrid(125, 188);
    kernel0 <<<k0_dimGrid, k0_dimBlock>>> (dev_M);
    cudaCheckKernel();
  }
  
  for (int c0 = 2; c0 <= 9998; c0 += 1)
    {
      dim3 k1_dimBlock(32);
      dim3 k1_dimGrid(125);
      kernel1 <<<k1_dimGrid, k1_dimBlock>>> (dev_M, c0);
      cudaCheckKernel();
    }
    
  cudaCheckReturn(cudaMemcpy(M, dev_M, (6000) * (4000) * sizeof(float), cudaMemcpyDeviceToHost));
  
  cudaUnbindTexture(texRef_MatchQ);
  
  cudaFreeArray(cuArr_MatchQ);
