...
#define cudaCheckKernel() \
  ...
  cudaMallocArray(&cuArr_A, &channelDesc_A, 4000, 4000);
  cudaMemcpyToArray(cuArr_A, 0, 0, A, (4000) * (4000) * sizeof(float), cudaMemcpyHostToDevice);
  ...
  for (int c0 = 0; c0 <= 4999; c0 += 1) {
    {
      dim3 k0_dimBlock(32);
      dim3 k0_dimGrid(125);
      kernel0 <<<k0_dimGrid, k0_dimBlock>>> (dev_x1, dev_x2, c0);
      cudaCheckKernel();
    }
    
    {
      dim3 k1_dimBlock(32);
      dim3 k1_dimGrid(125);
      kernel1 <<<k1_dimGrid, k1_dimBlock>>> (dev_x1, dev_x2, c0);
      cudaCheckKernel();
    }
