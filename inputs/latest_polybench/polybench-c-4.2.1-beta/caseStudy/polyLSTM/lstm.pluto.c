...
if (_PB_T >= 1) {
  for (t1=0;t1<=_PB_T-1;t1++) {
    if ((_PB_S >= 0) && (ns >= 0)) {
      lbp=0;
      ubp=floord(_PB_S+ns-1,32);
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6,t7,t8,t9,t10,t11,t12,t13,t14,t15,t16,t17)
...
#pragma vector always
    ...
    for (t4=0;t4<=floord(_PB_S-1,16);t4++) {
      lbp=max(0,ceild(32*t4-_PB_S+1,32));
      ubp=min(floord(_PB_S-1,32),t4);
#pragma omp parallel for private(lbv,ubv,t7,t8,t9,t10)
      for (t6=lbp;t6<=ubp;t6++) {
        for (t7=32*t4-32*t6;t7<=min(_PB_S-1,32*t4-32*t6+31);t7++) {
          lbv=32*t6;
          ubv=min(_PB_S-1,32*t6+31);
#pragma ivdep
#pragma vector always
