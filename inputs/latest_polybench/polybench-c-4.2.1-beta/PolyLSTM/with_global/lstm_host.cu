...
#define cudaCheckKernel() \
  do { \
    cudaCheckReturn(cudaGetLastError()); \
	  ...
	  float *dev_g;
	  float *dev_i;
	  float (*dev_inp_F)[3000];
	  float *dev_o;
	  float (*dev_s_F)[2850];
	  
	  cudaCheckReturn(cudaMalloc((void **) &dev_U_f, (2850) * (3000) * sizeof(float)));
	  cudaCheckReturn(cudaMalloc((void **) &dev_U_g, (2850) * (3000) * sizeof(float)));
	  cudaCheckReturn(cudaMalloc((void **) &dev_U_i, (2850) * (3000) * sizeof(float)));
	  cudaCheckReturn(cudaMalloc((void **) &dev_U_o, (2850) * (3000) * sizeof(float)));
	  cudaCheckReturn(cudaMalloc((void **) &dev_W_f, (2850) * (2850) * sizeof(float)));
	  cudaCheckReturn(cudaMalloc((void **) &dev_W_g, (2850) * (2850) * sizeof(float)));
	  cudaCheckReturn(cudaMalloc((void **) &dev_W_i, (2850) * (2850) * sizeof(float)));
	  cudaCheckReturn(cudaMalloc((void **) &dev_W_o, (2850) * (2850) * sizeof(float)));
	  ...
	  cudaCheckReturn(cudaMalloc((void **) &dev_inp_F, (400) * (3000) * sizeof(float)));
	  ...
	  cudaCheckReturn(cudaMemcpy(dev_U_f, U_f, (2850) * (3000) * sizeof(float), cudaMemcpyHostToDevice));
	  cudaCheckReturn(cudaMemcpy(dev_U_g, U_g, (2850) * (3000) * sizeof(float), cudaMemcpyHostToDevice));
	  cudaCheckReturn(cudaMemcpy(dev_U_i, U_i, (2850) * (3000) * sizeof(float), cudaMemcpyHostToDevice));
	  cudaCheckReturn(cudaMemcpy(dev_U_o, U_o, (2850) * (3000) * sizeof(float), cudaMemcpyHostToDevice));
	  cudaCheckReturn(cudaMemcpy(dev_W_f, W_f, (2850) * (2850) * sizeof(float), cudaMemcpyHostToDevice));
	  cudaCheckReturn(cudaMemcpy(dev_W_g, W_g, (2850) * (2850) * sizeof(float), cudaMemcpyHostToDevice));
	  cudaCheckReturn(cudaMemcpy(dev_W_i, W_i, (2850) * (2850) * sizeof(float), cudaMemcpyHostToDevice));
	  cudaCheckReturn(cudaMemcpy(dev_W_o, W_o, (2850) * (2850) * sizeof(float), cudaMemcpyHostToDevice));
	  if (ns >= 1)
	  ...
	  cudaCheckReturn(cudaMemcpy(dev_inp_F, inp_F, (400) * (3000) * sizeof(float), cudaMemcpyHostToDevice));
	  ...
	  for (int c0 = 0; c0 <= 400; c0 += 1) {
	    ...
	    if (c0 <= 399) {
	      ...
	      
	      {
	        dim3 k5_dimBlock(32);
	        dim3 k5_dimGrid(90);
	        kernel5 <<<k5_dimGrid, k5_dimBlock>>> (dev_U_f, dev_f, dev_inp_F, ns, c0);
	        cudaCheckKernel();
	      }
	      
	      {
	        dim3 k6_dimBlock(32);
	        dim3 k6_dimGrid(90);
	        kernel6 <<<k6_dimGrid, k6_dimBlock>>> (dev_U_g, dev_g, dev_inp_F, ns, c0);
	        cudaCheckKernel();
	      }
	      
	      {
	        dim3 k7_dimBlock(32);
	        dim3 k7_dimGrid(90);
	        kernel7 <<<k7_dimGrid, k7_dimBlock>>> (dev_U_i, dev_i, dev_inp_F, ns, c0);
	        cudaCheckKernel();
	      }
	      ...
	      if (c0 >= 1)
	        {
	          dim3 k8_dimBlock(32);
	          dim3 k8_dimGrid(90);
	          kernel8 <<<k8_dimGrid, k8_dimBlock>>> (dev_W_f, dev_f, dev_s_F, ns, c0);
	          cudaCheckKernel();
	        }
	        
	      {
	        dim3 k9_dimBlock(32);
	        dim3 k9_dimGrid(90);
	        kernel9 <<<k9_dimGrid, k9_dimBlock>>> (dev_U_o, dev_inp_F, dev_o, ns, c0);
	        cudaCheckKernel();
	      }
	      ...
	      if (c0 >= 1) {
	        {
	          ...
	          dim3 k10_dimGrid(90);
	          kernel10 <<<k10_dimGrid, k10_dimBlock>>> (dev_W_i, dev_i, dev_s_F, ns, c0);
	          cudaCheckKernel();
	        }
	        
	        {
	          ...
	          dim3 k11_dimGrid(90);
	          kernel11 <<<k11_dimGrid, k11_dimBlock>>> (dev_W_g, dev_g, dev_s_F, ns, c0);
	          cudaCheckKernel();
	        }
	        
	        {
	          ...
	          dim3 k12_dimGrid(90);
	          kernel12 <<<k12_dimGrid, k12_dimBlock>>> (dev_W_o, dev_o, dev_s_F, ns, c0);
	          cudaCheckKernel();
	        }
	  ...
	  cudaCheckReturn(cudaFree(dev_U_f));
	  cudaCheckReturn(cudaFree(dev_U_g));
	  cudaCheckReturn(cudaFree(dev_U_i));
	  cudaCheckReturn(cudaFree(dev_U_o));
	  cudaCheckReturn(cudaFree(dev_W_f));
	  cudaCheckReturn(cudaFree(dev_W_g));
	  cudaCheckReturn(cudaFree(dev_W_i));
	  cudaCheckReturn(cudaFree(dev_W_o));
