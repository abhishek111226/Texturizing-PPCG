...
#define cudaCheckKernel() \
  do { \
    cudaCheckReturn(cudaGetLastError()); \
	  ...
	  cudaArray* cuArr_s_F;
	  cudaChannelFormatDesc channelDesc_s_F= cudaCreateChannelDesc<float>();
	  cudaMallocArray(&cuArr_s_F, &channelDesc_s_F, 2850, ns <= 0 ? 399 : 400, cudaArraySurfaceLoadStore);
	  cudaMemcpyToArray(cuArr_s_F, 0, 0, s_F, (ns <= 0 ? 399 : 400) * (2850) * sizeof(float), cudaMemcpyHostToDevice);
	  cudaBindSurfaceToArray(surfRef_s_F, cuArr_s_F, channelDesc_s_F);
	    ...
	    if (ns >= 1 && c0 >= 1)
	      {
	        ...
	        kernel0 <<<k0_dimGrid, k0_dimBlock>>> (dev_c_F, dev_o, ns, c0);
	        cudaCheckKernel();
	      }
	      ...
	      if (c0 >= 1)
	        {
	          dim3 k8_dimBlock(32);
	          dim3 k8_dimGrid(90);
	          kernel8 <<<k8_dimGrid, k8_dimBlock>>> (dev_f, ns, c0);
	          cudaCheckKernel();
	        }
	      ...
	      if (c0 >= 1) {
	        {
	          ...
	          dim3 k10_dimGrid(90);
	          kernel10 <<<k10_dimGrid, k10_dimBlock>>> (dev_i, ns, c0);
	          cudaCheckKernel();
	        }
	        
	        {
	          ...
	          dim3 k11_dimGrid(90);
	          kernel11 <<<k11_dimGrid, k11_dimBlock>>> (dev_g, ns, c0);
	          cudaCheckKernel();
	        }
	        
	        {
	          ...
	          dim3 k12_dimGrid(90);
	          kernel12 <<<k12_dimGrid, k12_dimBlock>>> (dev_o, ns, c0);
	          cudaCheckKernel();
	        }
	  ...
	  cudaMemcpyFromArray(s_F, cuArr_s_F, 0, 0, (ns <= 0 ? 399 : 400) * (2850) * sizeof(float),  cudaMemcpyDeviceToHost);
