...
#define cudaCheckKernel() \
  do { \
    ...
    cudaArray* cuArr_x;
    cudaChannelFormatDesc channelDesc_x= cudaCreateChannelDesc<float>();
    cudaMallocArray(&cuArr_x, &channelDesc_x, 3, 8400);
    cudaMemcpyToArray(cuArr_x, 0, 0, x, (8400) * (3) * sizeof(float), cudaMemcpyHostToDevice);
    cudaBindTextureToArray(texRef_x, cuArr_x, channelDesc_x);
    
    cudaCheckReturn(cudaMemcpy(dev_f, f, (8400) * (3) * sizeof(float), cudaMemcpyHostToDevice));
    {
      dim3 k0_dimBlock(32);
      dim3 k0_dimGrid(263);
      kernel0 <<<k0_dimGrid, k0_dimBlock>>> (cutforcesq, epsilon, dev_f, sigma6);
      cudaCheckKernel();
    }
