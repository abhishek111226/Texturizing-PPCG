...
__global__ void kernel0(double cutforcesq, double epsilon, float f[8400][3], double sigma6, float x[8400][3])
{
    int b0 = blockIdx.x;
    int t0 = threadIdx.x;
    double private_fix;
    ...
    if (32 * b0 + t0 <= 8399) {
      private_xtmp = x[32 * b0 + t0][0];
      private_fix = 0;
      private_ztmp = x[32 * b0 + t0][2];
      private_ytmp = x[32 * b0 + t0][1];
      private_fiz = 0;
      private_fiy = 0;
      for (int c2 = 0; c2 < 32 * b0 + t0; c2 += 1) {
        private_delx = (private_xtmp - x[c2][0]);
        private_dely = (private_ytmp - x[c2][1]);
        private_delz = (private_ztmp - x[c2][2]);
