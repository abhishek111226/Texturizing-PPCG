...
int main(void) {
  ...
  if ((_nTimesteps >= 1) && (_nX >= 2) && (_nY >= 3)) {
    for (t1 = ceild(-_nY - 22, 16); t1 <= floord(3 * _nTimesteps - 5, 16);
         t1++) {
      lbp = max(max(ceild(t1 - 1, 3), ceild(8 * t1 - _nTimesteps + 3, 8)),
                -t1 - 2);
      ubp = min(min(floord(-4 * t1 + _nTimesteps - 1, 4),
                    floord(4 * t1 + _nY + 2, 12)),
                floord(_nTimesteps + _nY - 2, 16));
#pragma omp parallel for private(lbv, ubv, t3, t4, t5, t6)
      for (t2 = lbp; t2 <= ubp; t2++) {
        for (t3 = max(max(0, ceild(t1 + t2 - 3, 4)),
                      ceild(16 * t2 - _nY - 13, 16));
             t3 <= min(min(min(floord(_nTimesteps + _nX - 2, 16),
                               floord(16 * t2 + _nX + 12, 16)),
                           floord(4 * t1 + 4 * t2 + _nX + 10, 16)),
                       floord(8 * t1 - 8 * t2 + _nY + _nX + 5, 16));
             t3++) {
          for (t4 = max(max(max(max(0, 4 * t1 + 4 * t2), 8 * t1 - 8 * t2 + 2),
                            16 * t2 - _nY + 1),
                        16 * t3 - _nX + 1);
               t4 <= min(min(min(min(_nTimesteps - 1, 16 * t2 + 13),
                                 16 * t3 + 14),
                             4 * t1 + 4 * t2 + 11),
                         8 * t1 - 8 * t2 + _nY + 6);
               t4++) {
            ...
            if (t4 % 2 == 0) {
              for (t5 =
                       max(max(16 * t2, t4 + 2), -8 * t1 + 8 * t2 + 2 * t4 - 7);
                   t5 <= min(min(16 * t2 + 15, -8 * t1 + 8 * t2 + 2 * t4),
                             t4 + _nY - 1);
                   t5++) {
                lbv = max(16 * t3, t4 + 1);
                ubv = min(16 * t3 + 15, t4 + _nX - 1);
