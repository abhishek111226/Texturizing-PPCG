...
int main(void) {
  ...
  if ((_nTimesteps >= 1) && (_nX >= 2) && (_nY >= 3)) {
    for (t1 = -1; t1 <= floord(_nTimesteps - 1, 4); t1++) {
      lbp = max(ceild(t1, 2), ceild(8 * t1 - _nTimesteps + 3, 8));
      ubp = min(floord(_nTimesteps + _nY - 2, 8), floord(4 * t1 + _nY + 2, 8));
#pragma omp parallel for private(lbv, ubv, t3, t4, t5, t6)
      for (t2 = lbp; t2 <= ubp; t2++) {
        for (t3 = max(max(0, ceild(t1 - 1, 2)), ceild(8 * t2 - _nY - 5, 8));
             t3 <= min(min(min(floord(_nTimesteps + _nX - 2, 8),
                               floord(4 * t1 + _nX + 6, 8)),
                           floord(8 * t2 + _nX + 4, 8)),
                       floord(8 * t1 - 8 * t2 + _nY + _nX + 5, 8));
             t3++) {
          for (t4 = max(max(max(max(0, 4 * t1), 8 * t1 - 8 * t2 + 2),
                            8 * t2 - _nY + 1),
                        8 * t3 - _nX + 1);
               t4 <= min(min(min(min(_nTimesteps - 1, 4 * t1 + 7), 8 * t2 + 5),
                             8 * t3 + 6),
                         8 * t1 - 8 * t2 + _nY + 6);
               t4++) {
            ...
            if (t4 % 2 == 0) {
              for (t5 = max(max(8 * t2, t4 + 2), -8 * t1 + 8 * t2 + 2 * t4 - 7);
                   t5 <= min(min(8 * t2 + 7, -8 * t1 + 8 * t2 + 2 * t4),
                             t4 + _nY - 1);
                   t5++) {
                lbv = max(8 * t3, t4 + 1);
