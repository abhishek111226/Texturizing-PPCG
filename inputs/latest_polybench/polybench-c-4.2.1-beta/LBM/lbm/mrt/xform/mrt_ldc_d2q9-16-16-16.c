...
int main(void) {
  ...
  if ((_nTimesteps >= 1) && (_nX >= 2) && (_nY >= 3)) {
    for (t1 = -1; t1 <= floord(_nTimesteps - 1, 8); t1++) {
      ...
      ubp =
          min(floord(_nTimesteps + _nY - 2, 16), floord(8 * t1 + _nY + 6, 16));
#pragma omp parallel for private(lbv, ubv, t3, t4, t5, t6)
      for (t2 = lbp; t2 <= ubp; t2++) {
        for (t3 = max(max(0, ceild(t1 - 1, 2)), ceild(16 * t2 - _nY - 13, 16));
             t3 <= min(min(min(floord(_nTimesteps + _nX - 2, 16),
                               floord(8 * t1 + _nX + 14, 16)),
                           floord(16 * t2 + _nX + 12, 16)),
                       floord(16 * t1 - 16 * t2 + _nY + _nX + 13, 16));
             t3++) {
          for (t4 = max(max(max(max(0, 8 * t1), 16 * t1 - 16 * t2 + 2),
                            16 * t2 - _nY + 1),
                        16 * t3 - _nX + 1);
               t4 <=
                   min(min(min(min(_nTimesteps - 1, 8 * t1 + 15), 16 * t2 + 13),
                           16 * t3 + 14),
                       16 * t1 - 16 * t2 + _nY + 14);
               t4++) {
            ...
            if (t4 % 2 == 0) {
              for (t5 = max(max(16 * t2, t4 + 2),
                            -16 * t1 + 16 * t2 + 2 * t4 - 15);
                   t5 <= min(min(16 * t2 + 15, -16 * t1 + 16 * t2 + 2 * t4),
                             t4 + _nY - 1);
                   t5++) {
                lbv = max(16 * t3, t4 + 1);
