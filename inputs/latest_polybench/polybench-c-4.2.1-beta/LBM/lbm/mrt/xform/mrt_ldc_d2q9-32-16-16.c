...
int main(void) {
  ...
  if ((_nTimesteps >= 1) && (_nX >= 2) && (_nY >= 3)) {
    for (t1 = -1; t1 <= floord(3 * _nTimesteps + _nY - 4, 32); t1++) {
      lbp = max(max(ceild(2 * t1, 3), ceild(16 * t1 - _nTimesteps + 1, 8)),
                ceild(32 * t1 - _nTimesteps + 3, 32));
      ubp = min(min(floord(_nTimesteps + _nY - 2, 16),
                    floord(16 * t1 + _nY + 14, 24)),
                2 * t1 + 2);
      ...
      for (t2 = lbp; t2 <= ubp; t2++) {
        for (t3 = max(max(0, ceild(2 * t1 - t2 - 1, 2)),
             ...
             t3 <= min(min(min(floord(_nTimesteps + _nX - 2, 16),
                           ...
                           floord(16 * t1 - 8 * t2 + _nX + 22, 16)),
                       floord(32 * t1 - 32 * t2 + _nY + _nX + 29, 16));
             t3++) {
          for (t4 =
                   max(max(max(max(0, 16 * t1 - 8 * t2), 32 * t1 - 32 * t2 + 2),
                           16 * t2 - _nY + 1),
                       16 * t3 - _nX + 1);
            ...
            if (t4 % 2 == 0) {
              for (t5 = max(max(16 * t2, t4 + 2),
                            -32 * t1 + 32 * t2 + 2 * t4 - 31);
                   t5 <= min(min(16 * t2 + 15, -32 * t1 + 32 * t2 + 2 * t4),
                             t4 + _nY - 1);
                   t5++) {
                lbv = max(16 * t3, t4 + 1);
