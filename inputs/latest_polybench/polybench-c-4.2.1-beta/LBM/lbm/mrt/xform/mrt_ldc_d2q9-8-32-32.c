...
int main(void) {
  ...
  if ((_nTimesteps >= 1) && (_nX >= 2) && (_nY >= 3)) {
    for (t1 = ceild(-3 * _nY - 50, 32); t1 <= floord(5 * _nTimesteps - 11, 32);
         t1++) {
      lbp = max(max(ceild(-t1 - 4, 3), ceild(t1 - 3, 5)),
                ceild(8 * t1 - _nTimesteps + 3, 8));
      ubp = min(min(min(floord(8 * t1 + _nY + 6, 8),
                        floord(-4 * t1 + _nTimesteps - 1, 12)),
                    floord(4 * t1 + _nY + 2, 20)),
                floord(_nTimesteps + _nY - 2, 32));
#pragma omp parallel for private(lbv, ubv, t3, t4, t5, t6)
      for (t2 = lbp; t2 <= ubp; t2++) {
        for (t3 = max(max(0, ceild(t1 + 3 * t2 - 7, 8)),
                      ceild(32 * t2 - _nY - 29, 32));
             t3 <= min(min(min(floord(_nTimesteps + _nX - 2, 32),
                               floord(32 * t2 + _nX + 28, 32)),
                           floord(4 * t1 + 12 * t2 + _nX + 18, 32)),
                       floord(8 * t1 - 8 * t2 + _nY + _nX + 5, 32));
             t3++) {
          for (t4 = max(max(max(max(0, 4 * t1 + 12 * t2), 8 * t1 - 8 * t2 + 2),
                            32 * t2 - _nY + 1),
                        32 * t3 - _nX + 1);
               t4 <= min(min(min(min(_nTimesteps - 1, 32 * t2 + 29),
                                 32 * t3 + 30),
                             4 * t1 + 12 * t2 + 19),
                         8 * t1 - 8 * t2 + _nY + 6);
               t4++) {
            ...
            if (t4 % 2 == 0) {
              for (t5 =
                       max(max(32 * t2, t4 + 2), -8 * t1 + 8 * t2 + 2 * t4 - 7);
                   t5 <= min(min(32 * t2 + 31, -8 * t1 + 8 * t2 + 2 * t4),
                             t4 + _nY - 1);
                   t5++) {
                lbv = max(32 * t3, t4 + 1);
...
#pragma vector always
                for (t6 = lbv; t6 <= ubv; t6++) {
                  lbm_kernel(grid[0][(-t4 + t5)][(-t4 + t6)][0],
                             grid[0][(-t4 + t5) - 1][(-t4 + t6)][2],
                             grid[0][(-t4 + t5) + 1][(-t4 + t6)][4],
                             grid[0][(-t4 + t5)][(-t4 + t6) - 1][1],
                             grid[0][(-t4 + t5)][(-t4 + t6) + 1][3],
                             ...
                             &grid[1][(-t4 + t5)][(-t4 + t6)][8],
                             &grid[1][(-t4 + t5)][(-t4 + t6)][7], (t4),
                             ((-t4 + t5)), ((-t4 + t6)));
                  ;
                }
              }
...
#pragma vector always
                for (t6 = lbv; t6 <= ubv; t6++) {
                  lbm_kernel(grid[1][(-t4 + t5)][(-t4 + t6)][0],
                             grid[1][(-t4 + t5) - 1][(-t4 + t6)][2],
                             grid[1][(-t4 + t5) + 1][(-t4 + t6)][4],
                             grid[1][(-t4 + t5)][(-t4 + t6) - 1][1],
                             grid[1][(-t4 + t5)][(-t4 + t6) + 1][3],
                             ...
                             &grid[0][(-t4 + t5)][(-t4 + t6)][8],
                             &grid[0][(-t4 + t5)][(-t4 + t6)][7], (t4),
                             ((-t4 + t5)), ((-t4 + t6)));
                  ;
                }
              }
...
void lbm_kernel(double in_c, double in_n, double in_s, double in_e, double in_w,
  ...
  m_0 = +1 * in_c + 1 * in_e + 1 * in_n + 1 * in_w + 1 * in_s + 1 * in_ne +
        1 * in_nw + 1 * in_sw + 1 * in_se;
  m_1 = -1 * in_c - 1 * in_e - 1 * in_n - 1 * in_w + 2 * in_s + 2 * in_ne +
        2 * in_nw + 2 * in_sw - 4 * in_se;
  m_2 = -2 * in_c - 2 * in_e - 2 * in_n - 2 * in_w + 1 * in_s + 1 * in_ne +
        1 * in_nw + 1 * in_sw + 4 * in_se;
  m_3 = +1 * in_c + 0 * in_e - 1 * in_n + 0 * in_w + 1 * in_s - 1 * in_ne -
        1 * in_nw + 1 * in_sw + 0 * in_se;
  m_4 = -2 * in_c + 0 * in_e + 2 * in_n + 0 * in_w + 1 * in_s - 1 * in_ne -
        1 * in_nw + 1 * in_sw + 0 * in_se;
  m_5 = +0 * in_c + 1 * in_e + 0 * in_n - 1 * in_w + 1 * in_s + 1 * in_ne -
        1 * in_nw - 1 * in_sw + 0 * in_se;
  m_6 = +0 * in_c - 2 * in_e + 0 * in_n + 2 * in_w + 1 * in_s + 1 * in_ne -
        1 * in_nw - 1 * in_sw + 0 * in_se;
  m_7 = +1 * in_c - 1 * in_e + 1 * in_n - 1 * in_w + 0 * in_s + 0 * in_ne +
        0 * in_nw + 0 * in_sw + 0 * in_se;
  m_8 = +0 * in_c + 0 * in_e + 0 * in_n + 0 * in_w + 1 * in_s - 1 * in_ne +
        1 * in_nw - 1 * in_sw + 0 * in_se;
  ...
  *out_c = (1.0 / 36.0) *
           (+4 * m_eq0 - 4 * m_eq1 + 4 * m_eq2 + 0 * m_eq3 + 0 * m_eq4 +
            0 * m_eq5 + 0 * m_eq6 + 0 * m_eq7 + 0 * m_eq8);
  *out_e = (1.0 / 36.0) *
           (+4 * m_eq0 - 1 * m_eq1 - 2 * m_eq2 + 6 * m_eq3 - 6 * m_eq4 +
            0 * m_eq5 + 0 * m_eq6 + 9 * m_eq7 + 0 * m_eq8);
  *out_n = (1.0 / 36.0) *
           (+4 * m_eq0 - 1 * m_eq1 - 2 * m_eq2 + 0 * m_eq3 - 0 * m_eq4 +
            6 * m_eq5 - 6 * m_eq6 - 9 * m_eq7 + 0 * m_eq8);
  *out_w = (1.0 / 36.0) *
           (+4 * m_eq0 - 1 * m_eq1 - 2 * m_eq2 - 6 * m_eq3 + 6 * m_eq4 +
            0 * m_eq5 + 0 * m_eq6 + 9 * m_eq7 + 0 * m_eq8);
  *out_s = (1.0 / 36.0) *
           (+4 * m_eq0 - 1 * m_eq1 - 2 * m_eq2 + 0 * m_eq3 + 0 * m_eq4 -
            6 * m_eq5 + 6 * m_eq6 - 9 * m_eq7 + 0 * m_eq8);
  *out_ne = (1.0 / 36.0) *
            (+4 * m_eq0 + 2 * m_eq1 + 1 * m_eq2 + 6 * m_eq3 + 3 * m_eq4 +
             6 * m_eq5 + 3 * m_eq6 + 0 * m_eq7 + 9 * m_eq8);
  *out_nw = (1.0 / 36.0) *
            (+4 * m_eq0 + 2 * m_eq1 + 1 * m_eq2 - 6 * m_eq3 - 3 * m_eq4 +
             6 * m_eq5 + 3 * m_eq6 + 0 * m_eq7 - 9 * m_eq8);
  *out_sw = (1.0 / 36.0) *
