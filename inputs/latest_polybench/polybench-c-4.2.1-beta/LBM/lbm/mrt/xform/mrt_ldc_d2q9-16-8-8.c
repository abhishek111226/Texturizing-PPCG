...
int main(void) {
  ...
  if ((_nTimesteps >= 1) && (_nX >= 2) && (_nY >= 3)) {
    for (t1 = -1; t1 <= floord(3 * _nTimesteps + _nY - 4, 16); t1++) {
      lbp = max(max(ceild(2 * t1, 3), ceild(8 * t1 - _nTimesteps + 1, 4)),
                ceild(16 * t1 - _nTimesteps + 3, 16));
      ubp = min(
          min(floord(_nTimesteps + _nY - 2, 8), floord(8 * t1 + _nY + 6, 12)),
          2 * t1 + 2);
      ...
      for (t2 = lbp; t2 <= ubp; t2++) {
        for (t3 = max(max(0, ceild(2 * t1 - t2 - 1, 2)),
             ...
             t3 <= min(min(min(floord(_nTimesteps + _nX - 2, 8),
                       ...
                       floord(16 * t1 - 16 * t2 + _nY + _nX + 13, 8));
             t3++) {
          for (t4 = max(max(max(max(0, 8 * t1 - 4 * t2), 16 * t1 - 16 * t2 + 2),
                            8 * t2 - _nY + 1),
                        8 * t3 - _nX + 1);
               t4 <= min(min(min(min(_nTimesteps - 1, 8 * t2 + 5), 8 * t3 + 6),
                             8 * t1 - 4 * t2 + 11),
                         16 * t1 - 16 * t2 + _nY + 14);
               t4++) {
            ...
            if (t4 % 2 == 0) {
              for (t5 = max(max(8 * t2, t4 + 2),
                            -16 * t1 + 16 * t2 + 2 * t4 - 15);
                   t5 <= min(min(8 * t2 + 7, -16 * t1 + 16 * t2 + 2 * t4),
                             t4 + _nY - 1);
                   t5++) {
                lbv = max(8 * t3, t4 + 1);
