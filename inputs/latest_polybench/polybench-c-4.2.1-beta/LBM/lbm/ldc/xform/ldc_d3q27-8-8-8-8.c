...
int main(void) {
  ...
  if ((_nTimesteps >= 1) && (_nX >= 2) && (_nY >= 2) && (_nZ >= 3)) {
    for (t1 = -1; t1 <= floord(_nTimesteps - 1, 4); t1++) {
      ...
      ubp = min(floord(_nTimesteps + _nZ - 2, 8), floord(4 * t1 + _nZ + 2, 8));
#pragma omp parallel for private(lbv, ubv, t3, t4, t5, t6, t7, t8)
      for (t2 = lbp; t2 <= ubp; t2++) {
        for (t3 = max(max(0, ceild(t1 - 1, 2)), ceild(8 * t2 - _nZ - 5, 8));
             t3 <= min(min(min(floord(_nTimesteps + _nY - 2, 8),
                               floord(4 * t1 + _nY + 6, 8)),
                           floord(8 * t2 + _nY + 4, 8)),
                       floord(8 * t1 - 8 * t2 + _nZ + _nY + 5, 8));
             t3++) {
          for (t4 = max(
                   max(max(0, ceild(t1 - 1, 2)), ceild(8 * t2 - _nZ - 5, 8)),
                   ceild(8 * t3 - _nY - 5, 8));
               t4 <= min(min(min(min(floord(_nTimesteps + _nX - 2, 8),
                                     floord(4 * t1 + _nX + 6, 8)),
                                 floord(8 * t2 + _nX + 4, 8)),
                             floord(8 * t3 + _nX + 5, 8)),
                         floord(8 * t1 - 8 * t2 + _nZ + _nX + 5, 8));
               t4++) {
            for (t5 = max(max(max(max(max(0, 4 * t1), 8 * t1 - 8 * t2 + 2),
                                  8 * t2 - _nZ + 1),
                              8 * t3 - _nY + 1),
                          8 * t4 - _nX + 1);
                 t5 <= min(min(min(min(min(_nTimesteps - 1, 4 * t1 + 7),
                                       8 * t2 + 5),
                                   8 * t3 + 6),
                               8 * t4 + 6),
                           8 * t1 - 8 * t2 + _nZ + 6);
              ...
              if (t5 % 2 == 0) {
                for (t6 = max(max(8 * t2, t5 + 2),
                              -8 * t1 + 8 * t2 + 2 * t5 - 7);
                     t6 <= min(min(8 * t2 + 7, -8 * t1 + 8 * t2 + 2 * t5),
                               t5 + _nZ - 1);
                     t6++) {
