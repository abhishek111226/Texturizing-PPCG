...
int main(void) {
  ...
  if ((_nTimesteps >= 1) && (_nX >= 3) && (_nY >= 4)) {
    for (t1 = ceild(-_nY - 22, 16); t1 <= floord(3 * _nTimesteps - 6, 16);
         t1++) {
      lbp = max(max(ceild(t1 - 1, 3), ceild(8 * t1 - _nTimesteps + 4, 8)),
                -t1 - 2);
...
#pragma omp parallel for private(lbv, ubv, t3, t4, t5, t6)
      for (t2 = lbp; t2 <= ubp; t2++) {
        for (t3 = max(max(0, ceild(t1 + t2 - 3, 4)),
                      ceild(16 * t2 - _nY - 12, 16));
             t3 <= min(min(min(floord(_nTimesteps + _nX - 2, 16),
                               floord(16 * t2 + _nX + 11, 16)),
                           floord(4 * t1 + 4 * t2 + _nX + 10, 16)),
                       floord(8 * t1 - 8 * t2 + _nY + _nX + 5, 16));
             t3++) {
          for (t4 = max(max(max(max(0, 4 * t1 + 4 * t2), 8 * t1 - 8 * t2 + 3),
                            16 * t2 - _nY + 1),
                        16 * t3 - _nX + 1);
               t4 <= min(min(min(min(_nTimesteps - 1, 16 * t2 + 12),
                                 16 * t3 + 13),
                             4 * t1 + 4 * t2 + 11),
                         8 * t1 - 8 * t2 + _nY + 6);
               t4++) {
            ...
            if (t4 % 2 == 0) {
              for (t5 =
                       max(max(16 * t2, t4 + 3), -8 * t1 + 8 * t2 + 2 * t4 - 7);
