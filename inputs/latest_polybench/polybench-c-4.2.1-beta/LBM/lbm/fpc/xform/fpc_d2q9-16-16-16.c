...
int main(void) {
  ...
  if ((_nTimesteps >= 1) && (_nX >= 3) && (_nY >= 4)) {
    for (t1 = -1; t1 <= floord(_nTimesteps - 1, 8); t1++) {
      lbp = max(ceild(t1, 2), ceild(16 * t1 - _nTimesteps + 4, 16));
      ubp =
          min(floord(_nTimesteps + _nY - 2, 16), floord(8 * t1 + _nY + 6, 16));
#pragma omp parallel for private(lbv, ubv, t3, t4, t5, t6)
      for (t2 = lbp; t2 <= ubp; t2++) {
        for (t3 = max(max(0, ceild(t1 - 1, 2)), ceild(16 * t2 - _nY - 12, 16));
             t3 <= min(min(min(floord(_nTimesteps + _nX - 2, 16),
                               floord(8 * t1 + _nX + 14, 16)),
                           floord(16 * t2 + _nX + 11, 16)),
                       floord(16 * t1 - 16 * t2 + _nY + _nX + 13, 16));
             t3++) {
          for (t4 = max(max(max(max(0, 8 * t1), 16 * t1 - 16 * t2 + 3),
                            16 * t2 - _nY + 1),
                        16 * t3 - _nX + 1);
               t4 <=
                   min(min(min(min(_nTimesteps - 1, 8 * t1 + 15), 16 * t2 + 12),
                           16 * t3 + 13),
                       16 * t1 - 16 * t2 + _nY + 14);
               t4++) {
