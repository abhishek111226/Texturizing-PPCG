...
int main(void) {
  ...
  if ((_nTimesteps >= 1) && (_nX >= 3) && (_nY >= 4)) {
    for (t1 = ceild(-_nY - 46, 32); t1 <= floord(3 * _nTimesteps - 6, 32);
         t1++) {
      lbp = max(max(ceild(t1 - 1, 3), ceild(16 * t1 - _nTimesteps + 4, 16)),
                -t1 - 2);
...
#pragma omp parallel for private(lbv, ubv, t3, t4, t5, t6)
      for (t2 = lbp; t2 <= ubp; t2++) {
        for (t3 = max(max(0, ceild(t1 + t2 - 3, 4)),
             ...
             t3++) {
          for (t4 = max(max(max(max(0, 8 * t1 + 8 * t2), 16 * t1 - 16 * t2 + 3),
                            32 * t2 - _nY + 1),
                        32 * t3 - _nX + 1);
