...
int main(void) {
  ...
  if ((_nTimesteps >= 1) && (_nX >= 3) && (_nY >= 4)) {
    for (t1 = ceild(-3 * _nY - 50, 32); t1 <= floord(5 * _nTimesteps - 14, 32);
         t1++) {
      lbp = max(max(ceild(-t1 - 4, 3), ceild(t1 - 3, 5)),
                ceild(8 * t1 - _nTimesteps + 4, 8));
...
#pragma omp parallel for private(lbv, ubv, t3, t4, t5, t6)
      for (t2 = lbp; t2 <= ubp; t2++) {
        for (t3 = max(max(0, ceild(t1 + 3 * t2 - 7, 8)),
                      ceild(32 * t2 - _nY - 28, 32));
             t3 <= min(min(min(floord(_nTimesteps + _nX - 2, 32),
                               floord(32 * t2 + _nX + 27, 32)),
                           floord(4 * t1 + 12 * t2 + _nX + 18, 32)),
                       floord(8 * t1 - 8 * t2 + _nY + _nX + 5, 32));
             t3++) {
          for (t4 = max(max(max(max(0, 4 * t1 + 12 * t2), 8 * t1 - 8 * t2 + 3),
                            32 * t2 - _nY + 1),
                        32 * t3 - _nX + 1);
               t4 <= min(min(min(min(_nTimesteps - 1, 32 * t2 + 28),
                                 32 * t3 + 29),
                             4 * t1 + 12 * t2 + 19),
                         8 * t1 - 8 * t2 + _nY + 6);
               t4++) {
            ...
            if (t4 % 2 == 0) {
              for (t5 =
                       max(max(32 * t2, t4 + 3), -8 * t1 + 8 * t2 + 2 * t4 - 7);
