      ...
      if (32 * b0 + t0 <= 999)
        for (int c4 = t1; c4 <= ppcg_min(31, -32 * b1 + 1999); c4 += 16)
          for (int c5 = 0; c5 <= ppcg_min(31, -c2 + 999); c5 += 1)
            s_F[32 * b0 + t0][32 * b1 + c4] += ((tex2D(texRef_U, c2 + c5, 32 * b1 + c4)) * inp_F[32 * b0 + t0][c2 + c5]);
      __syncthreads();
    }
}
    ...
    for (int c2 = 0; c2 <= 1999; c2 += 32) {
      if (32 * b0 + t0 <= 1999)
        for (int c4 = 0; c4 <= ppcg_min(31, -c2 + 1999); c4 += 1)
          s_F[c0][32 * b0 + t0] += ((tex2D(texRef_W, c2 + c4, 32 * b0 + t0)) * s_F[c0 - 1][c2 + c4]);
      __syncthreads();
    }
}
      ...
      if (32 * b0 + t0 <= 999)
        for (int c4 = t1; c4 <= ppcg_min(31, -32 * b1 + 1499); c4 += 16)
          for (int c5 = 0; c5 <= ppcg_min(31, -c2 + 1999); c5 += 1)
            out_F[32 * b0 + t0][32 * b1 + c4] += ((tex2D(texRef_V, c2 + c5, 32 * b1 + c4)) * s_F[32 * b0 + t0][c2 + c5]);
      __syncthreads();
    }
}
