...
#define cudaCheckKernel() \
  do { \
    cudaCheckReturn(cudaGetLastError()); \
	  ...
	  cudaCheckReturn(cudaMalloc((void **) &dev_U, (2000) * (1000) * sizeof(float)));
	  cudaCheckReturn(cudaMalloc((void **) &dev_V, (1500) * (2000) * sizeof(float)));
	  cudaCheckReturn(cudaMalloc((void **) &dev_W, (2000) * (2000) * sizeof(float)));
	  ...
	  cudaCheckReturn(cudaMemcpy(dev_U, U, (2000) * (1000) * sizeof(float), cudaMemcpyHostToDevice));
	  cudaCheckReturn(cudaMemcpy(dev_V, V, (1500) * (2000) * sizeof(float), cudaMemcpyHostToDevice));
	  cudaCheckReturn(cudaMemcpy(dev_W, W, (2000) * (2000) * sizeof(float), cudaMemcpyHostToDevice));
	  ...
	  cudaCheckReturn(cudaMemcpy(dev_s_F, s_F, (1000) * (2000) * sizeof(float), cudaMemcpyHostToDevice));
	  {
	    ...
	    kernel0 <<<k0_dimGrid, k0_dimBlock>>> (dev_U, dev_inp_F, dev_s_F);
	    cudaCheckKernel();
	  }
	  ...
	  for (int c0 = 1; c0 <= 999; c0 += 1)
	    {
	      dim3 k1_dimBlock(32);
	      dim3 k1_dimGrid(63);
	      kernel1 <<<k1_dimGrid, k1_dimBlock>>> (dev_W, dev_s_F, c0);
	      cudaCheckKernel();
	    }
	    
	  {
	    ...
	    kernel2 <<<k2_dimGrid, k2_dimBlock>>> (dev_V, dev_out_F, dev_s_F);
	    cudaCheckKernel();
	  }
	  ...
	  cudaCheckReturn(cudaFree(dev_U));
	  cudaCheckReturn(cudaFree(dev_V));
