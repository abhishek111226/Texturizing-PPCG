...
__global__ void kernel0(float A[1200][1000], float C[1200][1200], float alpha, float beta)
{
      ...
      for (int c2 = 0; c2 <= 999; c2 += 32) {
        if (32 * b0 + t0 <= 1199)
          for (int c4 = t1; c4 <= ppcg_min(31, -c2 + 999); c4 += 16)
            shared_A_0[t0][c4] = A[32 * b0 + t0][c2 + c4];
        if (32 * b1 + t0 <= 1199)
          for (int c4 = t1; c4 <= ppcg_min(31, -c2 + 999); c4 += 16)
            shared_A_1[t0][c4] = A[32 * b1 + t0][c2 + c4];
        __syncthreads();
        if (32 * b0 + t0 <= 1199)
          for (int c4 = t1; c4 <= ppcg_min(31, 32 * b0 - 32 * b1 + t0); c4 += 16) {
            ...
            for (int c5 = 0; c5 <= ppcg_min(31, -c2 + 999); c5 += 1)
              C[32 * b0 + t0][32 * b1 + c4] += ((alpha * shared_A_0[t0][c5]) * shared_A_1[c4][c5]);
          }
        __syncthreads();
      }
}
