...
#define cudaCheckKernel() \
  do { \
    ...
    cudaCheckReturn(cudaMalloc((void **) &dev_B, (n - 1) * (1300) * sizeof(double)));
    
    
    cudaBindTexture(NULL,texRef_A, dev_A, (n) * (1300) * sizeof(double));
    
    cudaBindTexture(NULL,texRef_B, dev_B, (n - 1) * (1300) * sizeof(double));
    
    cudaCheckReturn(cudaMemcpy(dev_A, A, (n) * (1300) * sizeof(double), cudaMemcpyHostToDevice));
    cudaCheckReturn(cudaMemcpy(dev_B, B, (n - 1) * (1300) * sizeof(double), cudaMemcpyHostToDevice));
    {
      dim3 k0_dimBlock(16, 32);
      dim3 k0_dimGrid(ppcg_min(41, (n + 30) / 32), ppcg_min(256, (n + 30) / 32));
      kernel0 <<<k0_dimGrid, k0_dimBlock>>> (dev_A, dev_B, tsteps, n);
      cudaCheckKernel();
    }
    
    cudaCheckReturn(cudaMemcpy(B, dev_B, (n - 1) * (1300) * sizeof(double), cudaMemcpyDeviceToHost));
