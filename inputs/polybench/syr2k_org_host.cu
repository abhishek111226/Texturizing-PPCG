...
#define cudaCheckKernel() \
  do { \
    ...
    cudaCheckReturn(cudaMalloc((void **) &dev_B, (n) * (1000) * sizeof(double)));
    cudaCheckReturn(cudaMalloc((void **) &dev_C, (n) * (1200) * sizeof(double)));
    
    
    cudaBindTexture(NULL,texRef_A, dev_A, (n) * (1000) * sizeof(double));
    
    cudaBindTexture(NULL,texRef_B, dev_B, (n) * (1000) * sizeof(double));
    
    cudaBindTexture(NULL,texRef_C, dev_C, (n) * (1200) * sizeof(double));
    
    if (m >= 1) {
      cudaCheckReturn(cudaMemcpy(dev_A, A, (n) * (1000) * sizeof(double), cudaMemcpyHostToDevice));
      cudaCheckReturn(cudaMemcpy(dev_B, B, (n) * (1000) * sizeof(double), cudaMemcpyHostToDevice));
    }
