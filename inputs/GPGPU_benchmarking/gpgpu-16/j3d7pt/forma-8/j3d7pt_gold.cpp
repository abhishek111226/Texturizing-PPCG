...
{
//  const float (*input)[width_y][width_x] = (const float (*)[width_y][width_x])l_input;
...
//	  printf ("input[%d][%d][%d] = %.6f\n", i, j, k, input[i][j][k]);
  ...
  memset(temp, 0, sizeof(float) * height * width_y * width_x);
  j3d_step(l_output, height, width_y, width_x, temp, 4);
  memset(l_output, 0, sizeof(float) * height * width_y * width_x);
  j3d_step(temp, height, width_y, width_x, l_output, 5);
  memset(temp, 0, sizeof(float) * height * width_y * width_x);
  j3d_step(l_output, height, width_y, width_x, temp, 6);
  memset(l_output, 0, sizeof(float) * height * width_y * width_x);
  j3d_step(temp, height, width_y, width_x, l_output, 7);
